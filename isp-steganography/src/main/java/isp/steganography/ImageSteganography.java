...
public class ImageSteganography {
    ...
    public static void encode(final byte[] pt, final String inFile, final String outFile) throws IOException {
        ...
        final int paylength = pt.length;
        byte[] bytes = ByteBuffer.allocate(4).putInt(paylength).array();
        byte[] skupaj = new byte[bytes.length + paylength];
        System.arraycopy(bytes, 0, skupaj, 0, bytes.length);
        System.arraycopy(pt, 0, skupaj, bytes.length, pt.length);

        // Convert byte array to bit sequence
        final BitSet bits = BitSet.valueOf(skupaj);
     ...
     */
    public static byte[] decode(final String fileName, final int size) throws IOException {
        // load the image
